...
function analyzeWorkflows(workflows) {
  ...
  if (!hasPaymentFail) {
    issues.push({ severity: 'warning', title: 'No failed payment workflow', detail: 'If a client payment fails, there is no automated recovery process.', fix: 'Create a failed payment workflow: immediate notification email + SMS, retry reminders, and pause access after X days.' });
  }
  if (!hasAppointment) {
  ...
  if (generic.length > 2) {
    issues.push({ severity: 'warning', title: generic.length + ' workflows have generic names', detail: 'Workflows named Workflow 1 etc. are impossible to manage at scale.', fix: 'Rename all workflows to clearly describe their trigger and purpose.' });
  }
  if (draft.length > 5) {
    issues.push({ severity: 'warning', title: draft.length + ' workflows still in draft', detail: 'Many workflows are sitting in draft and not active.', fix: 'Review draft workflows - publish the ones that are ready, delete the rest.' });
  }
  ...
  const expert = active.length === 0
    ...
    : active.length < 5
    ? 'Only ' + active.length + ' active workflows is below the threshold for a production account.'
...
function analyzePipelines(pipelines, opportunities) {
  ...
  if (pipelines.length === 1) {
    issues.push({ severity: 'warning', title: 'Only one pipeline configured', detail: 'A single pipeline is usually not enough to handle both lead tracking and client lifecycle management.', fix: 'Consider adding a separate Client Lifecycle pipeline to separate pre-sale from post-sale.' });
  }
  const now = Date.now();
...
function analyzeForms(forms, surveys) {
  ...
  const score = Math.max(0, 100 - issues.filter(i => i.severity === 'critical').length * 40 - issues.filter(i => i.severity === 'warning').length * 15 - issues.filter(i => i.severity === 'suggestion').length * 5);
  const expert = forms.length + ' form(s) and ' + surveys.length + ' survey(s) found. Always confirm each form has a corresponding workflow trigger.';
  return { score: score, issues: issues, expert: expert, stats: { forms: forms.length, surveys: surveys.length } };
}
...
app.post('/api/audit', async function(req, res) {
  ...
  try {
    ...
    results.funnels = { score: score, issues: issues, expert: expert, stats: { total: funnels.length } };
  } catch (e) {
  ...
  res.json({ overallScore: overallScore, sections: results, allIssues: allIssues, errors: errors });
});
...
app.listen(PORT, function() {
  console.log('GHL Expert Audit running on port ' + PORT);
});
