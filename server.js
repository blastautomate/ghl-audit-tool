...
const GHL = axios.create({
  ...
  headers: { 'Version': '2021-07-28', 'Content-Type': 'application/json', 'Accept': 'application/json' },
  timeout: 15000
});

async function ghlGet(token, endpoint) {
  const clean = token.replace(/[\r\n\t]/g, '').trim();
  const res = await GHL.get(endpoint, { headers: { 'Authorization': 'Bearer ' + clean } });
  return res.data;
}
...
function analyzeWorkflows(workflows) {
  ...
  if (active.length === 0 && workflows.length === 0) issues.push({ severity: 'critical', title: 'No workflows found', detail: 'This account has zero workflows. All lead nurturing, onboarding, and follow-up automation is missing.', fix: 'Create essential workflows: lead nurture, appointment confirmation, onboarding series, and payment receipt.' });
  if (!hasOnboarding) issues.push({ severity: 'critical', title: 'Missing onboarding workflow', detail: 'No onboarding workflow detected. New clients are not being guided through your process automatically.', fix: 'Create a new client onboarding workflow: welcome email, resource delivery, next steps, and check-in sequence.' });
  if (!hasLeadFollowUp) issues.push({ severity: 'critical', title: 'Missing lead follow-up automation', detail: 'No lead follow-up or nurture sequence found. Leads that do not convert immediately are being lost.', fix: 'Build an after-intro-call workflow that sends follow-up emails based on whether the lead purchased or not.' });
  if (!hasPaymentFail) issues.push({ severity: 'warning', title: 'No failed payment workflow', detail: 'If a client payment fails, there is no automated recovery process.', fix: 'Create a failed payment workflow: immediate notification email + SMS, retry reminders, and pause access after X days.' });
  if (!hasAppointment) issues.push({ severity: 'critical', title: 'No appointment confirmation workflow', detail: 'No workflow to confirm, remind, or follow up on appointments. This directly causes no-shows.', fix: 'Create appointment automation: confirmation email on booking, 24hr reminder, 1hr SMS reminder, post-appointment follow-up.' });
  if (!hasUnsubscribe) issues.push({ severity: 'warning', title: 'No unsubscribe handling workflow', detail: 'Without an unsubscribe workflow, contact compliance may be at risk.', fix: 'Create an unsubscribe workflow that tags contacts and removes them from active campaigns immediately.' });
  if (!hasRenewal) issues.push({ severity: 'suggestion', title: 'No re-engagement or renewal workflow', detail: 'No workflows detected for re-engaging past clients or renewing subscriptions.', fix: 'Build a renewal reminder workflow 30/14/7 days before expiration, and a win-back sequence for churned clients.' });
  if (!hasReview) issues.push({ severity: 'suggestion', title: 'No review request workflow', detail: 'No automated review or reputation management workflow found.', fix: 'Add a post-service review request workflow that sends via SMS or email 24-48 hours after completion.' });
  ...
  const expert = active.length === 0 ? 'Zero workflows is a critical gap -- this account is running entirely manually.'
...
function analyzeContacts(contacts) {
  ...
  const expert = total + ' contacts with ' + emailPct + '% email coverage and ' + tagPct + '% tagging. ' + (tagPct < 40 ? 'Tagging hygiene needs attention.' : 'Tag coverage is reasonable.');
  return { score: score, issues: issues, expert: expert, stats: { total: total, withEmail: withEmail, withPhone: withPhone, withTags: withTags } };
}
...
function analyzePipelines(pipelines, opportunities) {
  ...
  const now = Date.now();
  const stale = opportunities.filter(o => { if (!o.updatedAt) return false; return (now - new Date(o.updatedAt).getTime()) / 86400000 > 30; });
  ...
  return { score: score, issues: issues, expert: expert, stats: { pipelines: pipelines.length, opportunities: opportunities.length, stale: stale.length } };
}
...
function analyzeCalendars(calendars) {
  ...
  return { score: score, issues: issues, expert: expert, stats: { total: calendars.length, withTeam: withTeam } };
}
...
function analyzeSettings(location) {
  ...
  const expert = 'Email configured: ' + (emailOk ? 'Yes' : 'No (!)') + '. Phone configured: ' + (phoneOk ? 'Yes' : 'No (!)') + '. ' + (!emailOk || !phoneOk ? 'Missing communication settings will silently break automations.' : 'Core communication channels are set up.');
  return { score: score, issues: issues, expert: expert, stats: { emailOk: emailOk, phoneOk: phoneOk, tzOk: tzOk, logoOk: logoOk, addrOk: addrOk } };
}
...
function analyzeCustomValues(customValues) {
  ...
  if (customValues.length > 0 && !customValues.some(cv => knownIntegrations.some(k => (cv.name || '').toLowerCase().includes(k)))) {
  ...
  const expert = customValues.length === 0 ? 'No custom values -- email personalization is limited to first name only.' : customValues.length + ' custom value(s) configured. Custom values are the data infrastructure for dynamic email and SMS content.';
  return { score: score, issues: issues, expert: expert, stats: { total: customValues.length } };
}
...
function buildAllIssues(sections) {
  ...
  return Object.entries(sections)
    .flatMap(function(e) { return (e[1].issues || []).map(function(i) { return Object.assign({}, i, { section: e[0] }); }); })
...
app.post('/api/audit', async function(req, res) {
  ...
  async function fetch(endpoint) { return ghlGet(token, endpoint); }

  try {
    const data = await fetch('/locations/' + locationId);
    ...
    const msg = e.response ? 'HTTP ' + e.response.status + ' on /locations/' + locationId : e.message;
    errors.settings = msg;
    results.settings = { score: 0, issues: [{ severity: 'critical', title: 'Could not fetch location settings', detail: msg, fix: 'Check that your token has locations.readonly scope.' }], expert: 'Could not fetch location settings.', stats: {} };
  }

  try {
    const data = await fetch('/contacts/?locationId=' + locationId + '&limit=100');
    results.contacts = analyzeContacts(data.contacts || []);
  } catch (e) {
    const msg = e.response ? 'HTTP ' + e.response.status + ' on /contacts' : e.message;
    errors.contacts = msg;
    results.contacts = { score: 0, issues: [{ severity: 'critical', title: 'Could not fetch contacts', detail: msg, fix: 'Ensure contacts.readonly scope is enabled.' }], expert: 'API error fetching contacts.', stats: { total: 0 } };
  }

  try {
    const pData = await fetch('/opportunities/pipelines/?locationId=' + locationId);
    const pipelines = pData.pipelines || [];
    let opportunities = [];
    try { const oData = await fetch('/opportunities/search/?location_id=' + locationId + '&limit=100'); opportunities = oData.opportunities || []; } catch (_) {}
    results.pipelines = analyzePipelines(pipelines, opportunities);
  } catch (e) {
    const msg = e.response ? 'HTTP ' + e.response.status + ' on /opportunities/pipelines' : e.message;
    errors.pipelines = msg;
    results.pipelines = { score: 0, issues: [{ severity: 'critical', title: 'Could not fetch pipelines', detail: msg, fix: 'Ensure opportunities.readonly scope is enabled.' }], expert: 'API error fetching pipelines.', stats: { pipelines: 0, opportunities: 0, stale: 0 } };
  }

  try {
    const data = await fetch('/workflows/?locationId=' + locationId);
    results.workflows = analyzeWorkflows(data.workflows || []);
  } catch (e) {
    const msg = e.response ? 'HTTP ' + e.response.status + ' on /workflows' : e.message;
    errors.workflows = msg;
    results.workflows = { score: 0, issues: [{ severity: 'critical', title: 'Could not fetch workflows', detail: msg, fix: 'Ensure workflows.readonly scope is enabled.' }], expert: 'API error fetching workflows.', stats: { total: 0, active: 0, draft: 0 } };
  }

  try {
    const data = await fetch('/calendars/?locationId=' + locationId);
    results.calendars = analyzeCalendars(data.calendars || []);
  } catch (e) {
    const msg = e.response ? 'HTTP ' + e.response.status + ' on /calendars' : e.message;
    errors.calendars = msg;
    results.calendars = { score: 0, issues: [{ severity: 'critical', title: 'Could not fetch calendars', detail: msg, fix: 'Ensure calendars.readonly scope is enabled.' }], expert: 'API error fetching calendars.', stats: { total: 0, withTeam: 0 } };
  }

  try {
    const fData = await fetch('/forms/?locationId=' + locationId + '&limit=50');
    let surveys = [];
    try { const sData = await fetch('/surveys/?locationId=' + locationId + '&limit=50'); surveys = sData.surveys || []; } catch (_) {}
    results.forms = analyzeForms(fData.forms || [], surveys);
  } catch (e) {
    const msg = e.response ? 'HTTP ' + e.response.status + ' on /forms' : e.message;
    errors.forms = msg;
    results.forms = { score: 0, issues: [{ severity: 'critical', title: 'Could not fetch forms', detail: msg, fix: 'Ensure forms.readonly scope is enabled.' }], expert: 'API error fetching forms.', stats: { forms: 0, surveys: 0 } };
  }

  try {
    const data = await fetch('/locations/' + locationId + '/customValues');
    results.customValues = analyzeCustomValues(data.customValues || []);
  } catch (e) {
    const msg = e.response ? 'HTTP ' + e.response.status + ' on /customValues' : e.message;
    errors.customValues = msg;
    results.customValues = { score: 50, issues: [{ severity: 'suggestion', title: 'Could not fetch custom values', detail: msg, fix: 'Ensure locations/customValues.readonly scope is enabled.' }], expert: 'Could not fetch custom values -- check API scopes.', stats: { total: 0 } };
  }

  try {
    const data = await fetch('/funnels/?locationId=' + locationId + '&limit=50');
    ...
    if (funnels.length === 0) {
      ...
      if (noDomain.length > 0) fIssues.push({ severity: 'warning', title: noDomain.length + ' funnel(s) may not have a custom domain', detail: 'Funnels without a custom domain use the default GHL URL which looks unprofessional.', fix: 'Connect a custom domain to each active funnel under Funnels > Settings > Domain.' });
    }
    const fScore = funnels.length === 0 ? 30 : Math.max(50, 100 - fIssues.filter(i => i.severity === 'warning').length * 15);
    results.funnels = { score: fScore, issues: fIssues, expert: funnels.length === 0 ? 'No funnels found -- every service business needs at minimum a lead capture page and a booking page.' : funnels.length + ' funnel/website(s) found. Verify each is on a custom domain and has a connected automation.', stats: { total: funnels.length } };
  } catch (e) {
    const msg = e.response ? 'HTTP ' + e.response.status + ' on /funnels' : e.message;
    errors.funnels = msg;
    results.funnels = { score: 50, issues: [{ severity: 'warning', title: 'Funnels scope needed', detail: msg, fix: 'Add funnels/funnel.readonly scope to your integration token.' }], expert: 'Funnels are your lead capture front door. Every service business needs at minimum: a lead magnet/opt-in page, a booking page, and a thank-you page with proper workflow triggers.', stats: { total: 0 } };
  }

  res.json({ overallScore: buildOverallScore(results), sections: results, allIssues: buildAllIssues(results), errors: errors });
});

app.get('*', function(req, res) { res.sendFile(path.join(__dirname, 'public', 'index.html')); });

const PORT = process.env.PORT || 3000;
app.listen(PORT, function() { console.log('GHL Expert Audit running on port ' + PORT); });
